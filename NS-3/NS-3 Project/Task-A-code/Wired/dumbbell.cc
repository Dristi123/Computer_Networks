...
main (int argc, char *argv[])
{
  ...
  bottleneckHelper.SetChannelAttribute ("Delay", StringValue ("3ms"));
  leftHelper.SetDeviceAttribute  ("DataRate", StringValue ("1000Mbps"));
  leftHelper.SetChannelAttribute ("Delay", StringValue ("3ms"));
  rightHelper.SetDeviceAttribute  ("DataRate", StringValue ("1000Mbps"));
  rightHelper.SetChannelAttribute ("Delay", StringValue ("3ms"));
  ...
  app->Setup (ns3TcpSocket, sinkAddress, 200, 10000000, DataRate ("1000Mbps"));
  ...
  firstsocket->TraceConnectWithoutContext ("CongestionWindow", MakeBoundCallback (&CwndChange, stream));
  ...
  Time delay;
  std::ofstream thr ("dthroughput.txt", std::ios::out | std::ios::app);
  std::ofstream pdr ("dpacket_delivery.txt", std::ios::out | std::ios::app);
  std::ofstream plr ("dpacket_loss.txt", std::ios::out | std::ios::app);
  std::ofstream dl ("ddelay.txt", std::ios::out | std::ios::app);
